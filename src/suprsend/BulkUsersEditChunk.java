...
class BulkUsersEditChunk {
	private static final Logger logger = Logger.getLogger(BulkUsersEditChunk.class.getName());
	...
	BulkUsersEditChunk(Suprsend config) {
		this.config = config;
	...
	private boolean checkLimitReached() {
		return (this.runningLength >= BulkUsersEditChunk.maxRecordsInChunk
				|| this.runningSize >= BulkUsersEditChunk.chunkApparentSizeInBytes);
	}
	...
	boolean tryToAddIntoChunk(JSONObject event, int eventSize) throws InputValueException {
		...
		if (this.runningSize + eventSize > BulkUsersEditChunk.chunkApparentSizeInBytes) {
			return false;
		}
		// Add Event to chunk
