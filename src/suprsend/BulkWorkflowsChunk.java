...
class BulkWorkflowsChunk {
    ...
    BulkWorkflowsChunk(Suprsend config) {
        this.config = config;
    ...
    private boolean checkLimitReached() {
        return (this.runningLength >= BulkWorkflowsChunk.maxRecordsInChunk || 
                this.runningSize >= BulkWorkflowsChunk.chunkApparentSizeInBytes);
    }
    ...
    private List<JSONObject> getFailedRecords(int statusCode, String errMsg) {
        return this.chunk.stream().map(
                c -> new JSONObject().put("record", c).put("error", errMsg).put("code", statusCode)
